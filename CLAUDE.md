...
A hub of short, professional-audience quizzes made to be shared on LinkedIn. Each
quiz is quick to take, self-scoring, and ends in a certificate image with
attribution baked in, so every share points back to the hub.
...
produces standalone HTML per quiz plus the hub, each with the engine inlined, so
every quiz is independently hostable and works on any static host.
