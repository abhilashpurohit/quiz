...
(function () {
  ...
  // quadrant certificate: name + tagline + a labelled 2x2 with the point plotted.
  function renderQuadrantCert(ctx, payload, cx, ACCENT, ACCENT_L, PAPER) {
    var g = payload.grid, ax = g.ax, innerW = 640 - 160;
    var serif = function (s) { return "700 " + s + "px Georgia, 'Times New Roman', serif"; };

    ctx.fillStyle = PAPER;
    var ns = fitFont(ctx, payload.code, innerW, 46, 30, serif);
    ctx.font = serif(ns);
    ctx.fillText(payload.code, cx, 186);

    ctx.fillStyle = ACCENT_L;
    ctx.font = "italic 700 20px Georgia, serif";
    ctx.fillText(payload.subhead, cx, 220);

    var G = 296, gx = cx - G / 2, gy = 268;
    // frame
    ...
    // faint quadrant names in their corners (x high = right, y high = top)
    ctx.fillStyle = "rgba(247,244,236,0.30)";
    ctx.font = "italic 13px Georgia, serif";
    ctx.textAlign = "center";
    var L = g.quadLabels;
    ctx.fillText(L[ax.x.lowKey + "-" + ax.y.highKey], gx + G * 0.26, gy + 20);
    ctx.fillText(L[ax.x.highKey + "-" + ax.y.highKey], gx + G * 0.74, gy + 20);
    ctx.fillText(L[ax.x.lowKey + "-" + ax.y.lowKey], gx + G * 0.26, gy + G - 12);
    ctx.fillText(L[ax.x.highKey + "-" + ax.y.lowKey], gx + G * 0.74, gy + G - 12);
    // pole labels outside the frame
    ctx.fillStyle = ACCENT_L;
    ...
    ctx.textAlign = "center";
    // the plotted point, inset so an extreme result sits just inside the frame
    var pad = 16, S2 = G - 2 * pad;
    var px = gx + pad + (g.xNorm + 1) / 2 * S2;
    var py = gy + pad + (1 - (g.yNorm + 1) / 2) * S2;
